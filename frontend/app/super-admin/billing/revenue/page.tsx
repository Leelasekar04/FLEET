...
// Mock data for the chart (Global Revenue vs Expenses)
const chartData = [
  { month: 'Jan', revenue: 145000, expenses: 88000 },
  { month: 'Feb', revenue: 152000, expenses: 91000 },
  { month: 'Mar', revenue: 148000, expenses: 89000 },
  { month: 'Apr', revenue: 161000, expenses: 94000 },
  { month: 'May', revenue: 159000, expenses: 92000 },
  { month: 'Jun', revenue: 175000, expenses: 98000 },
];
...
const recentTransactions = [
  { id: "INV-SUB-2026-001", company: "Kumar Transport Co.", type: "Enterprise Pro", amount: 1490.00, status: "paid", date: "2026-07-15" },
  { id: "INV-SUB-2026-002", company: "Swift Delivery Co.", type: "Standard", amount: 350.00, status: "pending", date: "2026-07-14" },
  { id: "INV-SUB-2026-003", company: "Apex Logistics", type: "Enterprise Custom", amount: 2999.00, status: "paid", date: "2026-07-12" },
  { id: "INV-SUB-2026-004", company: "City Cabs Inc.", type: "Overage Fee", amount: 125.50, status: "overdue", date: "2026-07-10" },
  { id: "INV-SUB-2026-005", company: "Global Freight", type: "Enterprise Pro", amount: 1490.00, status: "paid", date: "2026-07-09" },
];

export default function SuperAdminRevenueReportsPage() {
  const [dateRange, setDateRange] = useState("Last 6 Months");
  const [toastMessage, setToastMessage] = useState("");

  const handleExport = () => {
    setToastMessage("Generating global revenue report...");
    ...
    <div className="cmd-root p-6 max-w-[1600px] mx-auto">
      ...
      <div className="cmd-banner animate-fade-in">
        <div className="cmd-banner-left">
          <h1 className="cmd-banner-title">Platform Revenue</h1>
          <p className="cmd-banner-sub">
            Analyze global income streams, MRR across all tenants, and overall financial growth.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 animate-fade-in stagger-1">
        <div className="cmd-stat-card group hover:border-emerald-200 transition-colors">
          ...
          <div className="cmd-stat-label">Total Platform Revenue</div>
          <div className="cmd-stat-value text-slate-900">{fmt.currency(940000)}</div>
          <div className="flex items-center gap-1.5 mt-2 text-sm font-bold text-emerald-600 bg-emerald-50 w-fit px-2 py-0.5 rounded-md">
            <ArrowUpRight size={14} /> 15.2% vs last period
          </div>
        </div>
        ...
        <div className="cmd-stat-card group hover:border-blue-200 transition-colors">
          ...
          <div className="cmd-stat-label">Global MRR</div>
          <div className="cmd-stat-value text-slate-900">{fmt.currency(185000)}</div>
          <div className="flex items-center gap-1.5 mt-2 text-sm font-bold text-blue-600 bg-blue-50 w-fit px-2 py-0.5 rounded-md">
            <ArrowUpRight size={14} /> 6.8% growth
          </div>
        </div>
        ...
        <div className="cmd-stat-card group hover:border-amber-200 transition-colors">
          ...
          <div className="cmd-stat-label">Total Pending Subscriptions</div>
          <div className="cmd-stat-value text-slate-900">{fmt.currency(32450)}</div>
      ...
      <div className="cmd-chart-card mt-6 animate-fade-in stagger-2 p-6">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-lg font-black text-slate-900 tracking-tight">Revenue vs Platform Expenses</h2>
            <p className="text-sm font-medium text-slate-500 mt-1">Global financial overview for {dateRange.toLowerCase()}</p>
          </div>
      ...
      <div className="cmd-chart-card mt-6 animate-fade-in stagger-3">
        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-base font-black text-slate-900 tracking-tight">Recent Tenant Subscriptions</h2>
